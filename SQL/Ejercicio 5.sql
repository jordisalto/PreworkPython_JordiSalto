...
SELECT clientes.nombre,  pedidos.cliente_id
FROM public.clientes
INNER JOIN public.pedidos ON pedidos.cliente_id=clientes.id

/*22. Realiza una consulta para obtener todos los clientes y sus pedidos
correspondientes utilizando un left join.*/
SELECT clientes.nombre,  pedidos.cliente_id
FROM public.clientes
LEFT JOIN public.pedidos ON pedidos.cliente_id=clientes.id
...
INNER JOIN public.detallespedido ON productos.id=detallespedido.producto_id

/*24. Realiza una consulta para obtener todos los productos y los detalles de pedido
correspondientes utilizando un left join.*/
SELECT productos.nombre,  detallespedido.pedido_id
FROM public.productos
LEFT JOIN public.detallespedido ON productos.id=detallespedido.producto_id
...
datos a entero.*/
ALTER TABLE public.Clientes
ADD telefono_nuevo INT;
ALTER TABLE public.Clientes
DROP COLUMN telefono;
ALTER TABLE public.Clientes
RENAME COLUMN telefono_nuevo TO telefono;

/*7. Elimina la columna "telefono" de la tabla "Clientes".*/
ALTER TABLE public.Clientes
DROP COLUMN telefono;
...
RENAME COLUMN nombre TO nombre_complejo;

/*30. Agrega una restricción de clave primaria a la columna "id" en la tabla "Usuarios".*/
ALTER TABLE Usuarios
ADD CONSTRAINT PK_Usuarios PRIMARY KEY (id);
/*Aunque relamente no hace falta hacerlo, porque se ha definido como clave primaria al definr la tabla*/